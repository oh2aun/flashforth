...
; pack>  ( flash-addr ram-addr -- ) ; unpack 18 bytes link+length+name
...
; pack>  ( flash-addr ram-addr length -- ) ; pack 18 bytes link+length+name
        .pword  paddr(CUNPACK_L)+PFLASH
CPACK_L:
        .byte   NFA|3
        .ascii  ">>f"
        .align  2
CPACK:
        ...
        mov     [W14--], W0
        mov     [W14--], W1
        mov     W1, Preg            ; ram addr Preg
		push	W0
CPACK1:
        mov     [W14++], [W14]      ; dup flash addr in tos
		mov		Preg, W1
		mov		[W1++], [++W14]
        mov.b   [W1],[++W14]
        mov.b   [W1],[++W14]        ; Upper byte will be ignored in CF!
        rcall   ROT
		rcall	CFSTORE
		inc2	Preg
		rcall	PCFETCH
...
CREATE:
        rcall   BL
        rcall   WORD
        mov     [W14++], [W14]      ; Remember word at rhere
        rcall   FIND
        rcall   SWOP
        sub     W14, #2, W14
...
LEAVE:
        pop     W0  
        clr     [W15++]   ; TODO: Does this really work ?
        return
